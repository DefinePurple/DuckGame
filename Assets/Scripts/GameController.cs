...
public class GameController : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int playerIndex = 0; playerIndex < sessionInfo.GetPlayerList().Count; playerIndex++) { // loop through all players
            ...
            newPlayer.transform.SetParent(parent.transform); //add the player to the panel parent
            ...
            playerList[playerIndex] = newPlayer.GetComponent<Test_Player>(); // add the new tank object to the corresponding reference in the list
        }
        #endregion
    }
    ...
    public void ReceiveShot(int peerId, int senderPeerId) {
        ...
        for (int i = 0; i < playerList.Length; i++) {
            if (playerList[i].GetPeerId() == senderPeerId) {
                //***********//
                //**FOR SAM**//
                //***********//
                //Do the shoot animation and nothing else
            }

            if (playerList[i].GetPeerId() == peerId && playerList[i].CheckIsPlayer()) {
